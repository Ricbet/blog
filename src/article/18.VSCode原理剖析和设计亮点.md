...
然后就是 VS Code 的文件模块加载机制了

我们所知道的模块加载机制有 CommonJS、AMD 以及 ES6 模块化等

**CommonJS**

运行机制是需要提前编译处理且加载是同步的。以文件为单位的模块机制，有自己的作用域避免了全局污染，使用时必须指定 `module.exports` 的属性有哪些，类似与声明该模块有哪些接口方法可以给使用方调用

而在引入时是通过 `require(XXX)` 的方式加载，本质上此时读取到的该模块上的方法只是值的拷贝，模块内部的变化并不会影响到使用方

**AMD**

与 `CommonJS` 不同，AMD 可以动态的加载模块并支持指定回调函数，它是异步的。而 `vscode-loader` 就是对 AMD 模块的一个封装，专门服务于 VS Code 自身的文件模块加载


**vscode-loader**

[vscode-loader](https://github.com/microsoft/vscode-loader) 本质上还是 AMD 模块加载方式，只不过是服务于 VS Code 自身，还支持 CSS 和多语言 NLS，也支持匿名模块的加载。应用于 Electron 中时如果遇到无法通过 AMD 加载的模块则会委派给 `CommonJS`。
